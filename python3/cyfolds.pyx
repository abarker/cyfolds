...
cdef (bint, bint) line_begins_fun_or_class_def(line: str, prev_nested: cy.int,
    ...
    if prev_nested or in_string:
        return False, False
    matchobject = fold_keywords_matcher.match(line, pos=indent_spaces,)

    begins_def: bint
    begins_def = bool(matchobject)
    begins_class_def = line and begins_def and line[indent_spaces:indent_spaces+2] == "cl"
...
cdef void calculate_foldlevels(foldlevel_list: List[cy.int], buffer_lines: List[str],
    ...
    a single pass through the text.  The return values are by way of the `foldlevel_list`
    passed in."""
